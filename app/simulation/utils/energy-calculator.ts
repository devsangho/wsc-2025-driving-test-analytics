...
export interface EnergyConsumptionResult {
  ...
  inverterLoss: number;           // 인버터 손실 (kWh)
}
...
export function calculateSegmentEnergyConsumption(
  ...
  // 인버터 손실 (5% 손실 가정)
  const inverterLoss = motorEnergyConsumed * 0.05;
  
  // 총 에너지 소비량
  const totalEnergyConsumed = motorEnergyConsumed + auxiliaryEnergyConsumed + inverterLoss;
...
export function calculateSegmentEnergyProduction(
  ...
  const solarEnergyProduced = dayTotalEnergy * drivingTimeRatio * mpptEfficiency;
...
export function updateBatteryLevel(
  ...
  // 배터리 레벨 업데이트 (5-100% 사이로 제한) - 배터리가 5% 이하로 떨어지지 않도록 안전장치 추가
  const batteryLevelBefore = batteryLevel;
  let batteryLevelAfter = batteryLevel + batteryPercentChange;
  
  // 배터리가 위험 수준 이하로 떨어질 경우 (5% 이하), 비상 충전 적용
  if (batteryLevelAfter < 5) {
    console.log('Battery critically low, emergency charging activated');
    // 안전을 위해 최소 10%까지 설정
    batteryLevelAfter = 10;
  }
  
  // 최종 배터리 레벨 (0-100% 범위로 제한)
  batteryLevelAfter = Math.min(100, Math.max(5, batteryLevelAfter));
...
export async function calculateMorningCharge(
  ...
  try {
    ...
    // 아침 발전량 계산 (kWh)
    const morningChargeEnergy = morningChargeData.reduce(
    ...
    // 발전량을 배터리 충전에 적용
    const newBatteryLevel = calculateNewSOC(batteryLevel, actualMorningCharge, 0);
    
    console.log(`Morning charge: +${actualMorningCharge.toFixed(2)}kWh, Battery: ${newBatteryLevel.toFixed(1)}%`);
    
    return newBatteryLevel;
  } catch (error) {
    console.warn("Failed to calculate morning charge, using estimate instead:", error);
